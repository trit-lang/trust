...
| `bootstrap/` | the lexer and the expression parser, written in Trust. `scripts/bootstrap.sh` runs it and the Rust one over the same corpus and compares them token for token — 600 so far, over every literal form §1.4 defines and the places two lexers stop agreeing — holds nine more that both must **refuse**, compared on which character is wrong rather than on the wording, twenty expressions, fifteen whole functions, the items of nine files — `mod`, `use`, `type`, `const`, `impl` with receivers, struct literals, paths, `match` and its patterns — and **every file of `bootstrap/` itself**: the Trust parser reads its own source, 126 items of it, and prints the tree the Rust parser prints whose *trees* both must print identically, three whole **programs** — the driver bundles the module tree onto stdin, because the machine has no filesystem (G9.40) — and the **symbol table** of three more: pass one of Ch. 6 §4, `kind path pub`, which is the first thing a second implementation of that chapter can be held to because it needs no types and no lowering (`trust symbols` against `bootstrap/symbols.tr`), and pass two — what every `use` reaches, and which of the chapter's two rules refused it when it reached nothing (`trust uses` against `bootstrap/uses.tr`; `bootstrap/programs/refused` is every way one can fail), and pass three — the whole program as one flat list of items with every name in it rewritten to what it names (`trust flat` against `bootstrap/flat.tr`, which resolves its own source). **Ch. 2** is next to it: `trust layout` against `bootstrap/sizes.tr` — sizes, alignments, field offsets, discriminants and where each enum keeps its tag, over a corpus that reaches every one of §5's four answers |
