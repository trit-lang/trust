...
| `bootstrap/` | the lexer, written in Trust. `scripts/bootstrap.sh` runs it and the Rust one over the same corpus and compares them token for token — 219 so far. Written to be *used*: two bugs and a wart came out of its first hundred lines (G9.35, G9.36) |
