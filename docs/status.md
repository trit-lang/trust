...
| `lang::index` | an outline, go-to-definition, hover, find-references and rename, from the AST alone — so both work on a file that does not compile. Names resolve by scope and spelling; what needs a type is left unanswered rather than guessed (G0.5) |
