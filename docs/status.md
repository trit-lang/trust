...
| `bootstrap/` | the lexer, written in Trust. `scripts/bootstrap.sh` runs it and the Rust one over the same corpus and compares them token for token — 600 so far, over every literal form §1.4 defines and the places two lexers stop agreeing — and holds nine more that both must **refuse**, compared on which character is wrong rather than on the wording. Written to be *used*: two bugs and a wart came out of its first hundred lines (G9.35, G9.36) |
