...
//! # What this pass does
//!
//! One transformation, `promote_slots`: a `slot` whose every use is a `load`
//! or `store` **through it**, all within one block, becomes an SSA value.
//! Stores are deleted and each load is replaced by whatever was last stored.
//!
//! # Why one block
//!
